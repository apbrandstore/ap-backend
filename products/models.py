...
class TrackingCode(models.Model):
    """Model for storing tracking codes (Facebook Pixel, Google Analytics, etc.)"""
    name = models.CharField(
        max_length=100,
        help_text='Name of the tracking code (e.g., Facebook Pixel, Google Analytics)'
    )
    code = models.TextField(
        help_text='Paste the tracking code here (e.g., Facebook Pixel base code)'
    )
    is_active = models.BooleanField(
        default=True,
        help_text='Whether this tracking code is currently active'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
